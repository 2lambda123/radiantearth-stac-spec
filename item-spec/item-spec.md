...
| collection   | STRONGLY RECOMMENDED. URL to a Collection. _Absolute_ URLs should be used whenever possible. The referenced Collection is STRONGLY RECOMMENDED to implement the same STAC version as the Item. A link with this `rel` type is _required_ if the `collection` field in properties is present. |
